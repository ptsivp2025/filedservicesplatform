...
/**
 * Thumbnail kecil TERPISAH dari foto utama - dipakai galeri/daftar (grid
 * 56-112px) yang SEBELUMNYA memuat foto full 1600px cuma untuk ditampilkan
 * sekecil ibu jari, lalu di-scale turun oleh CSS. Egress-nya sama besar
 * dengan membuka foto penuh walau yang terlihat cuma thumbnail.
 *
 * Dipanggil dengan file yang SUDAH lewat compressImage() (bukan file asli
 * dari kamera) - mendekode ulang <1MB jauh lebih murah daripada mendekode
 * foto asli 4-8MB dua kali per upload.
 *
 * null kalau bukan gambar atau proses gagal - pemanggil tinggal skip
 * thumb_path (fallback ke foto penuh), upload tetap jalan.
 */
export async function makeThumbnail(file: File, opts: CompressOptions = {}): Promise<File | null> {
  if (!file.type.startsWith('image/') || SKIP_TYPES.has(file.type)) return null;

  const maxDim = opts.maxDim ?? THUMB_MAX_DIM;
  ...
  try {
    ...
    const newName = file.name.replace(/\.[^.]+$/, '') + '-thumb.jpg';
    return new File([blob], newName, { type: 'image/jpeg', lastModified: Date.now() });
  } catch {
    return null;
  }
