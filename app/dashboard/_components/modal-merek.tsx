...
import {
  ...
  unggahBerkasMerek, angkaTembus, gradasiPanelLogin, gradasiLatarPlatform,
} from '@/lib/merek';
      ...
      <section className="rounded-2xl border border-slate-200 overflow-hidden">
        ...
        <div className="p-4 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <Warna label="Warna Aksen (nama portal + garis sidebar)" nilai={form.warnaAksen} onChange={v => ubah('warnaAksen', v)} />
          </div>

          <div>
            <Warna label="Warna Sidebar / Menu Bar" nilai={form.warnaSidebar} onChange={v => ubah('warnaSidebar', v)} />
            <p className="text-[11px] text-slate-400 mt-1.5 leading-relaxed">
              Boleh hex (#1e1b2e) atau rgba(...) — kolom teks di samping color-picker menerima keduanya. Bawaan putih tembus; kalau diganti warna gelap, teks &amp; ikon menu tetap gelap, jadi pilih warna yang cukup terang supaya tetap terbaca.
            </p>
          </div>

          <div>
            <Label>Pratinjau Latar Platform</Label>
            <div className="mt-1.5 h-16 rounded-xl border border-slate-200"
              style={form.gambarLatarDasbor ? { backgroundImage: `url(${form.gambarLatarDasbor})`, backgroundSize: 'cover', backgroundPosition: 'center' } : { background: gradasiLatarPlatform(form) }} />
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <Warna label="Warna Latar Platform (awal)" nilai={form.warnaLatarAwal} onChange={v => ubah('warnaLatarAwal', v)} />
            <Warna label="Warna Latar Platform (akhir)" nilai={form.warnaLatarAkhir} onChange={v => ubah('warnaLatarAkhir', v)} />
          </div>
          <p className="text-[11px] text-slate-400 -mt-2 leading-relaxed">
            Gradasi ini jadi latar belakang Dashboard, Field Service, dan Teknisi — HANYA dipakai kalau "Gambar Latar Dashboard" di bawah dikosongkan. Isi gambar untuk memakai foto sebagai gantinya.
          </p>

          <Unggah label="Gambar Latar Dashboard" jenis="latarDasbor" nilai={form.gambarLatarDasbor} sedang={mengunggah === 'latarDasbor'}
            keterangan="Opsional — kalau diisi, foto ini dipakai sebagai latar Dashboard/Field Service/Teknisi menggantikan gradasi warna di atas. Maks 8MB, dikecilkan otomatis ke 2400px."
      ...
      <section className="rounded-2xl border border-slate-200 overflow-hidden">
        ...
        <div className="px-4 pt-4">
          ...
          <div className="mt-1.5 rounded-xl overflow-hidden border border-slate-200 flex h-44 bg-cover bg-center"
            style={form.gambarLatar ? { backgroundImage: `url(${form.gambarLatar})` } : undefined}>
            ...
            <div className="w-1/2 flex items-center justify-center p-4"
              style={{ background: form.gambarLatar ? `rgba(255,255,255,${angkaTembus(form.tembusKanan, 0.55)})` : '#f8fafc' }}>
        ...
        <div className="p-4 space-y-4">
          <Unggah label="Gambar Latar" jenis="latar" nilai={form.gambarLatar} sedang={mengunggah === 'latar'}
            keterangan="Opsional — kalau diisi, foto ini dipakai sebagai latar halaman login. Kosong (bawaan) = panel kiri tampil gradasi Warna Panel Kiri di bawah, pekat penuh, tanpa foto. Maks 8MB, dikecilkan otomatis ke 2400px."
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <Geser label="Kepekatan Panel Kiri" nilai={form.tembusLogin} bawaan={0.84}
              catatan="Hanya berlaku kalau Gambar Latar diisi. Makin kecil, foto latarnya makin terlihat menembus warna."
              onChange={v => ubah('tembusLogin', v)} />
            <Geser label="Kabut Putih Sisi Kanan" nilai={form.tembusKanan} bawaan={0.55}
              catatan="Hanya berlaku kalau Gambar Latar diisi. Makin besar, sisi kartu login makin putih dan tulisannya makin terbaca."
